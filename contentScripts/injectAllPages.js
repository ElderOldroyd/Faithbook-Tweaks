...
const hideProfileSelector = `img:not(.njmm-override):not(._1ift):not([alt^="Image"]):not(.scaledImageFitWidth):not([src*="rsrc"]), image:not(.njmm-override):not([src*="rsrc"]),.uiScaledImageContainer._26w6, svg:not(.njmm-override)`;
...
function startHidingAllProfilePicturesAndOtherThings() {
  ...
  browser.storage.local.get("allImages").then((result) => {
    if (result.allImages) {
      installCSS(hideImagesCSS);
    }
  }, onError);
  browser.storage.local.get("videos").then((result) => {
    if (result.videos) {
      installCSS(hideVideosCSS);
    }
  }, onError); //broken
  browser.storage.local.get("newsfeed").then((result) => {
    if (result.newsfeed) {
      installCSS(hideNewsfeedCSS);
    }
  }, onError);
...
function addEventListenerToRecieveRequest() {
  ...
  browser.runtime.onMessage.addListener((requestFromBrowserAction) => {
    ...
    if (
      requestFromBrowserAction.request ===
      "The browserAction would like your page's outerHTML"
    ) {
...
function notifyExtension(e) {
  ...
  if (document.querySelector('[aria-label*="Add"]')) {
    //There are Add Buttons on the page
    ...
    //There are Undo Buttons on the page
    browser.runtime.sendMessage({ undoButtons: true }); //send message
  } else if (window.location.href.includes("search")) {
    //Even if there are not Add Buttons, if we search, we will want to put the buttons on search pages.
    browser.runtime.sendMessage({ addButtons: true }); //send message
  } else {
    return; //do nothing
  }
